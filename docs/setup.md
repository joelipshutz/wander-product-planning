...
- Builds `0.1 (1)` through `0.1 (7)` uploaded successfully and began App Store Connect processing. Build `0.1 (7)` is the next M5 Add link-capture QA candidate.
...
- Build `0.1 (7)` is attached to `Wander Alpha`, export compliance is set to `usesNonExemptEncryption=false`, and external TestFlight review is `WAITING_FOR_REVIEW`.
