...
| Does Clerk's default iOS token work for Supabase RLS? | Blocked on hosted Supabase third-party verifier config. Do not expand M4 sync until this passes. | 2026-06-02 API smoke minted a default Clerk session token with `sub=<clerk user id>`, `role=authenticated`, `iss=https://growing-pheasant-22.clerk.accounts.dev`, `alg=RS256`, and a `kid` present in Clerk JWKS. Supabase still rejected it before RLS with `PGRST301 No suitable key was found to decode the JWT`. Supabase own anon JWT decodes and reaches function grants, so RPC exposure is not the blocker. Next: inspect Supabase dashboard Third-Party Auth / Clerk provider activation or wait/retry provider key refresh; avoid switching to the deprecated JWT-template path unless native third-party auth cannot be made to work. |
